...
**Trigger (the cleaner refactor, still deferred):** the next time upload-field plumbing is touched, or a third nesting-capable container is added. `collectionPath` is constant for the whole form, so the prop-drilling is redundant threading. Expose it on `FormProvider` / the form-context store (it currently lives only inside the `PendingUpload` interface, not on the store) and have `FileField` / `ImageField` read it via `useFormContext()`, dropping the `collectionPath` prop from `FieldRenderer`, `ArrayField`, and `GroupField` entirely. Low urgency — the prop-drilling fix is correct and complete; this only removes plumbing.
