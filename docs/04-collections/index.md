...
Ordinary `afterCreate` and `afterUpdate` failures are also post-commit. Core preserves the SDK's rejection contract with `ERR_DOCUMENT_HOOK_COMMITTED`, carrying only the failed phase, committed document/version IDs, and an allowlisted side-effect error code. The admin host treats that rejection as a saved document with a follow-up warning and refreshes canonical state. Do not retry the create or update itself: a create can duplicate the document, and an update can mint an unnecessary immutable version.
