...
Version-creating `afterCreate` and `afterUpdate` failures are also post-commit. Core preserves the SDK's rejection contract with `ERR_DOCUMENT_HOOK_COMMITTED`, carrying only the failed phase, committed document/version IDs, and an allowlisted side-effect error code. This includes ordinary create/update, duplicate, restore, copy-to-locale, and delete-locale operations; singleton `afterSave` uses the same contract. The admin host treats that rejection as a saved document with a follow-up warning and refreshes canonical state. Do not retry the write itself: a create or duplicate can create another document, while update-like operations can mint an unnecessary immutable version.
