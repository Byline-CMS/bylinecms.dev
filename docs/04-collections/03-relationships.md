...
- **The nested-object DSL was chosen over dot notation** (`'author.id': 'X'`). Dot notation collides with Byline's internal EAV path notation and doesn't absorb the `hasMany` quantifiers (`some`, `every`, `none`).
