...
The modal-time picker writes a tentative `{ title, path }` envelope so the in-editor display has a label immediately, but it is **not** the authoritative shape: the server walker rewrites it (or marks it unresolved) on save. Both save-time refresh and read-time population resolve targets in the originating actor's context. The framework asserts the target collection's `read` ability, applies its strict `beforeRead` predicate, runs `afterRead`, and preserves the inherited read mode; embed refreshes deliberately use the published view with public locale visibility, so an editor's save never copies a draft or an unchecked translation of the target into the stored snapshot. Each target resolves its language under its own collection's policy: a checked Spanish translation on the source document does not authorize the target's unchecked Spanish (see [Public delivery of advertised locales](../08-internationalization/03-content-locales.md#public-delivery-of-advertised-locales)).

:::note[Snapshots keep copied values until the next save]
With the default snapshot policy, a document stores the target's `title` and `path` as they were at its last save. If the target later changes — it is unpublished, deleted, or an editor unchecks the translation the snapshot copied — the stored snapshot keeps the old values until the source document is saved again. Enable `populateRelationsOnRead` on a field when its links must follow the target's current state on every read. Clearing caches does not repair a stored snapshot.
:::
...
This is the single source of truth for how a document addresses publicly. `CollectionAdminConfig.preview.url` should delegate to it (see `apps/webapp/byline/collections/pages/admin.tsx` for the pattern). The walker calls `buildDocumentPath` inside a `try`: if it throws, the walker logs at `info` level, removes the old copied `document.path`, and marks the link unresolved (branch A).

**Three branches, per link node.** Save-time embed and read-time populate use the same visitor, so both follow these branches. Copied values are replaced, never merged: a value the current target result does not supply is removed rather than kept from an earlier copy.

| Branch | Trigger | Effect on `document` |
|---|---|---|
| **Found** | Target resolved; `buildDocumentPath` (or generic fallback) returned a string | `title` ← `target.fields[useAsTitle]` (removed when the target has none); `path` ← canonical leading-slash path; any prior `_resolved` flag cleared |
| **A: no path** | `buildDocumentPath` raised, or no usable path could be composed | Log at `info` when the hook threw; delete `path`; set `_resolved: false`. The renderer renders the link text without an `<a>`. A stale path is never reused |
...
- **Inline image** — `{ title, altText, image, sizes }`. `image` is the source media's `StoredFileValue`; `sizes` is `deriveImageSizes(image.variants)` flattened into a renderer-friendly `{ name, url, width, height, format }[]`. Top-level `src` / `width` / `height` / `altText` are also persisted on the inline-image node: Lexical needs them to render in the admin editor. Those are the *editor preview*, not a copy of the original file. The picker writes the position-aware variant `getPreferredSize` selects (`left` / `right` → `card`, `wide` → `desktop`, otherwise `tablet`, with the original as the fallback) into `src` together with that variant's dimensions, so the URL and the dimensions are a matched pair. The visitor refreshes all three through the same function, which keeps the pair matched and stops a re-keyed or regenerated upload serving a stale URL to the editor. Every copied media value is replaced, never merged: when the media target is missing or denied to the reader, or resolves without an image or a usable URL, the visitor removes the copied `title`, `altText`, `image`, and `sizes`, sets `src` to an empty string, removes `width` and `height`, and sets `_resolved: false`, so no stale image is rendered or exported to Markdown. The node's position, caption, and click-through link are left alone. Public rendering never reads them — it renders from `document.image` and `document.sizes`, and takes the `<img>` intrinsic `width` / `height` from the file value's own `imageWidth` / `imageHeight`.

- **Inline-image click target** — an inline image optionally carries a *second*, independent relation under `node.link`, holding the same `LinkAttributes` union the link node uses. The flat envelope on the node addresses the source media document; `link` addresses a click-through target, which is an unrelated document in an unrelated collection. It is nested rather than flattened for exactly that reason. `linkType: 'internal'` carries a `{ title?, path?, _resolved?: false }` envelope refreshed by `inlineImageLinkVisitor`; `linkType: 'custom'` carries a literal URL and has no relation to resolve. An absent `link` means the image is not clickable.
...
**What the factories actually do.** Each composes every Lexical plugin's visitor into a single function. The package ships three visitors today, shared between the two trigger points. Note that there are three visitors but only two node types: an inline image carries two independent relations, and a `PendingHydration` addresses exactly one `{ collectionPath, documentId }` pair, so its click target needs a visitor of its own:
...
| `inlineImageVisitor` | `extensions/inline-image/populate.ts` | Refreshes `node.document` with `{ title, altText, image, sizes }`, plus the editor-preview `src` / `width` / `height` from the position-aware `getPreferredSize` variant — only when that yields a non-blank URL | No-op (no explicit miss handler; node keeps its picker-time envelope and existing preview) |
| `inlineImageLinkVisitor` | `extensions/inline-image/populate.ts` | Refreshes `node.link.document.title` / `.path` for an inline image's optional click target, using the same internal-link rules as `linkVisitor` (shared via `createInternalLinkHydration`). Skips custom-URL links | Deletes `title` / `path`, sets `_resolved: false` — the image keeps rendering, only its anchor is dropped |

All three visitors are pure / framework-agnostic: no React, no DOM, no Lexical runtime. They live next to the plugin's UI code so each plugin's write-time embed and read-time populate stay in lockstep, but only the visitor module is reachable from the package's `server` entry. The shared driver (`runLexicalPopulate`) walks the value's Lexical tree once per call and dispatches across every visitor in a single pass — a node that matches more than one visitor, as a linked inline image does, enqueues one pending hydration per match. Pending target ids are batched per source collection and handed to core's secure reader.
