...
Once the application is running, [Configuration](./03-configuration.md) explains every application-owned file under `apps/webapp/byline`, which runtime imports it, and where each kind of customization belongs. Existing 3.21 installations should first follow [Upgrading from 3.21 to 4.x](./04-upgrading-to-v4.md); 4.11 installations should then follow [Upgrading from 4.11 to 4.12](./05-upgrading-to-v4-12.md). Before moving a 4.19 installation to v5, follow the fenced [v5 upgrade guide](./06-upgrading-to-v5.md) and its provider-specific cutover runbook. A 5.2 installation moving to v6 should then follow [Upgrading from 5.2 to 6.x](./09-upgrading-to-v6.md), which covers the search-schema repair and the richtext capability change.
