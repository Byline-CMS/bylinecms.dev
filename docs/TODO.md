...
The domain home is [AUDIT.md](./AUDIT.md) — four workstreams closing the gap between the public auditability claim ("who wrote it, who changed it") and the admin surfaces. W1: the version audit trail (acting user + action) — `document_versions.created_by` exists but is never written; wire `requestContext.actor.id` through every `createDocumentVersion` call site, surface as `createdBy` (raw column, sibling of `updatedAt`), display labels batch-resolved admin-side as an `actors` map, and render the audit record in a framework-owned **audit strip** sub-row (History view default-on; list views toggleable) — keeping `listViewColumns` a user-fields surface. (Vocabulary: "audit", never "attribution" — the latter is the public copyright/credit concept; see AUDIT.md.) W2: the audit log table + migration, with transitive per-document read gating and a separate `admin.activity.read` gate for the system-wide log (subsumes the former standalone entry below). W3: content-versions / document-history split on the history route (tabs-vs-child-routes parked). W4: system activity area — new admin menu item + route with a filterable report over the version stream + audit log union. W1 is migration-free and ships first (**shipped in v3.8.0** — history-view audit strip; list-view strip deferred). **W2's transaction prerequisite shipped in v3.9.0:** the request-scoped `withTransaction` boundary (AsyncLocalStorage propagation, adapter-owned capability) is live in `@byline/db-postgres` — see [TRANSACTIONS.md](./TRANSACTIONS.md). **W2 + W3 shipped in v3.10.0:** the `byline_audit_log` table (folded into the squashed `0000` Drizzle baseline; existing-site production script `sql/0003_add-audit-log.sql`), `audit.append` / `getDocumentAuditLog` on the contract, the three write-points (`updateDocumentSystemFields`, `changeStatus`, `deleteDocument`) recording rows atomically, the gated `CollectionHandle.auditLog()` read + host server fn, and the **Document history** tab (single-route + `tab` search param). **W4 shipped on `develop` (unreleased):** the system activity area — `IAuditQueries.findAuditLog` (version-stream + audit-log `UNION ALL`), the `getSystemActivityLog` host server fn behind a new `admin.activity.read` ability, the `/admin/activity` route + factory, and the **Activity** menu item with collection / action / date-range filters. **All four workstreams are now complete**; this entry can be pruned from "Now" at the next release. (Also in v3.9.0: a fix so synthetic non-UUID script/seed actors write NULL `created_by` rather than crashing — a v3.8.0 W1 regression.)
...
**Folded into the auditability work domain (Now) — see [AUDIT.md](./AUDIT.md)** Workstreams 2 and 3, which carry this entry's spec forward (audit-log table written from `updateDocumentSystemFields` / `changeStatus` under the existing auth gate; tabbed History view). The original phase sketch remains at [CORE-DOCUMENT-STORAGE.md → Phase — document-grain audit log](./CORE-DOCUMENT-STORAGE.md#phase--document-grain-audit-log).
