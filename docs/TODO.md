...
The Byline client config (`defineClientConfig`, run as a side-effect of importing `apps/webapp/byline/admin.config.ts`) is registered from **two** points on the `_byline` route, because neither alone is sufficient: `route.tsx` `beforeLoad` (a dynamic import — registers before any `_byline/*` child *loader* reads `getClientConfig()`, closing a dev race where the loader outran the lazy module) **and** `route.lazy.tsx`'s side-effect import (registers for component render / initial hydration, where `beforeLoad` is not re-run). It works but is awkward — two entry points, both keeping the heavy admin/editor graph out of public bundles. Collapsing them to a **single eager point** (a top-level import in the route tree) only works if `admin.config`'s static import graph is light enough that eager-importing it doesn't bloat public-route bundles.

**Groundwork shipped (richtext side):** `@byline/richtext-lexical` now exposes a light `./config` subpath (`lexicalEditor`, the built-in extension *names*, the light toolbar-authoring primitives — no `RichTextField`/`Nodes`/`EditorField` or heavy content-extension classes), plus name-based `extensions.remove/has/replace` so a built-in can be toggled without importing its class. `@byline/ai`'s `AiLexicalExtension` is now statically light too (the AI drawer loads only via dynamic import). `byline/admin.config` + `fields/lexical-richtext-ai` consume `./config`. So referencing the **editor runtime** no longer pulls it into the importing bundle.

**The actual blocker (admin side, not richtext):** `admin.config` still can't go eager, because every collection admin config statically imports presentation components from `@byline/admin/react` — e.g. `DateTimeFormatter` (in every collection), and `MediaListView` → `LocalDateTime`. That barrel is a **deliberately indivisible single-Context-identity unit** (`FormContext`, `FieldServicesContext`, `AdminServicesContext`, `NavigationGuardContext`); its own header explains that per-area subpath exports break React Context identity under bundlers that pre-bundle subpaths individually (Vite `optimizeDeps.include`). The column formatters are *live references* held by the config objects, so they can't be tree-shaken away. Eager-importing `admin.config` therefore drags the whole admin **document-editor** surface (FormRenderer, FieldRenderer, every field widget, DiffModal) into public-route bundles — the exact regression the dual registration exists to avoid. This generalises: **any** admin component wired into a column view or custom slot has this effect whenever it comes from (or transitively imports) `@byline/admin/react`; light formatters that render off `@byline/core` types only (e.g. `FeaturedFormatter`, `MediaThumbnail`) do not.

**Real prerequisite:** package the light admin-presentation primitives (column formatters, badges, date/locale renderers) so collection admin configs can reference them *without* pulling the editor surface — and without reintroducing the Context-identity trap the single barrel exists to prevent. That's a packaging/design effort, not a refactor; until it lands, the dual registration stays. Touches `@byline/admin` packaging + the `_byline` route files (`route.tsx`, `route.lazy.tsx`) and the registration comments in `client.tsx` / `admin.config.ts` / `CLAUDE.md`.
