...
#### ~~Benchmark the UNION ALL at scale~~ — done 2026-04-18
Sweep run on M1 Pro at 1k / 10k / 50k / 100k scales. Full results at [`benchmarks/storage/results/2026-04-18-storage-cold-summary.md`](../../benchmarks/storage/results/2026-04-18-storage-cold-summary.md). Highlights: `getDocumentById` holds at ~3 ms median full-reconstruct across all scales (the UNION ALL cost is bounded by fields-per-doc, not docs-in-collection); `getDocumentsByDocumentIds` (batch 50) and `populateDocuments` depth-2 both stay flat at ~7 ms and ~3 ms respectively; the only query type that scales with N is the list view (`findDocuments` grows 6 → 17 → 70 → 129 ms), driven by the `current_documents` window function rather than the per-document reconstruct.

#### ~~Consider a read cache~~ — closed as not needed 2026-04-18
The JSONB read-cache column would be targeting `getDocumentById`, which the benchmark shows takes ~3 ms at 100k documents. The upper bound on any cache-column optimisation is replacing 3 ms with less — not enough to justify the complexity of maintaining a denormalised cache, especially with source-layer memoisation on `document_version_id` eliminating near-all production hits. If a future workload surfaces different numbers, the benchmark harness makes the reopen trivial.

#### List-view materialisation — deferred, reopen when a real workload demands it
The actual scaling pressure is on `findDocuments` at 50k+ (70 ms and up). The right optimisation if that becomes load-bearing is **materialising `current_documents` as a table** (trigger-maintained or periodically refreshed), not a per-row JSONB cache. Not worth building on speculation; the benchmark makes future reassessment cheap.
...
`IDocumentQueries.findDocuments()` accepts a declarative spec (document-level filters, `DocumentFilter[]` — a discriminated union of `FieldFilter` and `RelationFilter` — compiled into EXISTS subqueries (`RelationFilter` becomes a depth-scoped nested EXISTS joining `store_relation` to the target's current-documents view), `FieldSort` compiled into `LEFT JOIN LATERAL`, and selective field loading). `@byline/client`'s `parse-where.ts` translates the client DSL (including nested-object sub-wheres on relation fields) into these descriptors via the shared `fieldTypeToStore` mapping in `@byline/core`.
...
#### Status-aware reads (Phase 5) — shipped 2026-04-18
`status?: 'published' | 'any'` on every read method in `@byline/client` (defaulting to `'published'`) maps to a new adapter-level `readMode?: ReadMode` parameter threaded through `findDocuments`, `getDocumentById`, `getDocumentByPath`, `getDocumentsByDocumentIds`, and `populateDocuments`. Backed by a new `current_published_documents` Postgres view whose window function runs *after* filtering `document_versions` to `status = 'published'` — so a draft saved over a previously-published version does not hide the published content. Admin code paths keep working unchanged (the adapter default is `'any'`). This closes the last read-side gap that was blocking `@byline/client` use by public consumers.
...
The architecture is defensible and genuinely interesting. The dual flatten/reconstruct issue has been resolved — both paths now use the same schema-aware `storage-utils.ts` implementation. The template queries have been replaced with a generated column manifest, and selective field loading is now a first-class capability. Query paths have been unified, search is now configurable per collection, and locale copy-forward runs in a single DB round trip. Phases 1 through 5 of the client API have shipped (read, filter, sort, populate, write, status-aware reads); the populate DSL is a two-axis scope + projection model (`true` / `'*'` / `PopulateMap` at every level) with a unified relation envelope that preserves link metadata through populate; `useAsTitle` lives on the schema so server-side consumers can read a document's identity without a UI dependency. The `ReadContext` primitive pre-empts the A→B→A recursion class ahead of the future `afterRead` hook work. Phase 4 completed the in-process SDK surface — `CollectionHandle` now exposes `create` / `update` / `delete` / `changeStatus` / `unpublish` as thin shims over `document-lifecycle`. Phase 5 closed the last read-side gap for public consumers: a new `current_published_documents` view gives draft-over-published documents the correct fallback semantics, and `@byline/client` defaults every read to `'published'` mode. The UNION-ALL-at-scale benchmark ran on 2026-04-18 and closed both that strategic item and its follow-on "consider a read cache" item — single-doc reads hold at ~3 ms all the way to 100k documents, populate batch fetches stay flat, and the only query type that scales with document count is the list view (driven by the `current_documents` window function, not the UNION ALL). With storage performance characterised and the client API surface complete through Phase 5, the immediate priorities are user-facing features (`hasMany` relations, `afterRead` hook) rather than storage-layer work.
...
| 2026-04-18 | @byline/client Phase 5 — status-aware reads | New `ReadMode` (`'published' \| 'any'`) on `IDocumentQueries` (`findDocuments`, `getDocumentById`, `getDocumentByPath`, `getDocumentsByDocumentIds`) and `populateDocuments`. New `current_published_documents` Postgres view filters `document_versions` to `status = 'published'` before the ROW_NUMBER partition, giving public readers a valid fallback past newer drafts. `@byline/client` read methods expose `status?: 'published' \| 'any'` (defaulting to `'published'`). Admin paths keep working — adapter default is `'any'`. 10 unit + 7 integration tests. Migration `0001_demonic_joseph.sql`. |
