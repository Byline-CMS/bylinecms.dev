...
For version-creating collection and singleton operations, core reports this rejection as `ERR_DOCUMENT_HOOK_COMMITTED`. The contract covers ordinary saves, duplicate, restore, copy-to-locale, and delete-locale. The admin host recognizes the committed outcome, refreshes or navigates to the canonical document, clears form dirty state where applicable, and warns that dependent content may remain stale. SDK callers should reconcile the failed side effect or rebuild the index; they must not repeat the versioned write. Status and system-field hook failures can likewise reject after their mutations have committed, with their operation-specific reconciliation behavior.
