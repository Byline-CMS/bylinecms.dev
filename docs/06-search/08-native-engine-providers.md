...
No native-engine provider ships with Byline today. A Solr provider is under development against a production installation and is expected to upstream once it meets the conformance bar above; the Solr sketch in the [provider contract](./04-provider-contract.md#portable-and-native-analysis) reflects that design. Semantic and vector retrieval remain reserved capability flags with no built-in implementation.
