...
- [Experimental CLI](./getting-started/experimental-cli.md) — install Byline into an existing TanStack Start application with `byline init` (plus `setup` and `doctor`).
- [Development environment and example application](./getting-started/development-environment.md) — clone this repo, provision PostgreSQL, seed the database, and run the example app (`apps/webapp`) in dev mode.
