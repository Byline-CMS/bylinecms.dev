...
For now every locale's bundle is part of the initial admin JS payload (~5 kB
...
build time. The design names ~5 locales as the point where lazy locale loading
earns its complexity — **that threshold is now crossed**: the admin ships seven
interface locales (`en`, `fr`, `es`, `de`, `it`, `zh-CN`, `ko`) as of 2026-06-14,
so the eager payload is ~35 kB of admin-only JS (it never reaches the public
bundle — the admin graph is code-split out via the `_byline` lazy route). Still
low-severity, but no longer hypothetical (see [Remaining work](#remaining-work)
and the TODO entry).
...
than gated, accountability for them is the job of the document-grain audit log
(shipped v3.10.0 — see [AUDIT.md](./AUDIT.md)) rather than the version history.
...
- **Lazy locale loading** — replace the eager static-import bundle map with async
  loaders. The ~5-locale threshold is now crossed (seven interface locales ship as
  of 2026-06-14), so this is a live payload-weight item rather than a hypothetical;
  severity stays low (~5 kB gzipped per locale, admin-only). Design surface and
  trigger are captured in `docs/TODO.md`.
