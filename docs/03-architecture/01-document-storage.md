...
Immutable versioning gives content changes a complete, queryable history: every save is a new `document_versions` row, and the admin **History** view renders that lineage with per-version diffs. Two classes of change deliberately sit **outside** that version stream — this is the [document-grain vs version-grain](./index.md#3-document-grain-vs-version-grain) split described in the architecture overview:

1. **Document-grain system fields** — `path` (`byline_document_paths`), the editorial `availableLocales` set (`byline_document_available_locales`), and the tree edge (`byline_document_relationships`). These are edited through dedicated **non-versioned** writes (`updateDocumentPath`, `setDocumentAvailableLocales`, `placeTreeNode`) that mint no version and don't reset status. They are document-grain and sticky across versions, so gating them behind the publish workflow would falsely imply per-version staging (see [Internationalization](../07-internationalization/index.md), [Document Paths](../04-collections/04-document-paths.md), and [Document Trees](../04-collections/03-document-trees.md)).
