...
Because these changes are immediate rather than versioned, accountability for them is the job of the document-level audit log: a `byline_audit_log` table records who changed the path, advertised locales, or status — and when, and from what to what — written from the same service entry points under the existing `assertActorCanPerform` gate. The admin surfaces it as a **Document history** tab beside the version timeline. See [Auditability](../06-auth-and-security/02-auditability.md) for the full reference.
