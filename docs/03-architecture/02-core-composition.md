...
Companions:
- [AUTHN-AUTHZ.md](../06-auth-and-security/01-authn-authz.md) — the ability-key design that the future `createCommand({ auth: { abilities } })` slot will consume.
- [ROUTING-API.md](../05-reading-and-delivery/02-routing-and-api.md) — admin server fns are the current call sites that would benefit from a command-tree shape.
- [RICHTEXT.md](../04-collections/06-rich-text.md) — the richtext adapter is the first server-side adapter slot on `ServerConfig.fields.*`; pattern for future field-level server adapters.
