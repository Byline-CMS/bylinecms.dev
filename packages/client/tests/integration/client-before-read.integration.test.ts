 ...
 * `docs/07-auth-and-security/01-authn-authz.md` — Recipe 1 (owner-only drafts) is wired
...
describe('beforeRead — audit-log access gate', () => {
  it("returns empty when the actor's predicate excludes the document", async () => {
    ...
    // mirroring the history gate exactly (docs/07-auth-and-security/02-auditability.md — W2 authorization).
