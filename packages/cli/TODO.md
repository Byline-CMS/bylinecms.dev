...
| scaffold  | ✅ done         | Auto-mode. Two-tree overlay: `src/templates/byline/` (always-copied minimal) + `src/templates/byline-examples/` (full lift of `apps/webapp/byline/` with portability patches). Examples overlay supersedes matching base files. Non-destructive — skips files that already exist; also adds the app-local type generation/check scripts to `package.json`. |
...
| wire      | ✅ done         | Five independent sub-edits under `src/phases/wire/` (`server-ts`, `start-ts`, `byline-layout-tsx`, `tsconfig`, `vite-config`). Each reports `done | skipped | manual | blocked` into `state.wireSubEdits`. ts-morph drives the three import-injection edits; jsonc-parser drives the comment-preserving tsconfig edit; vite is byte-compared against canonical at `src/templates/host/vite.config.ts` (manual + snippet on mismatch — never auto-merged). Phase returns `partial` if any sub-edit is `manual`. **Canonical updated 2026-05-04** to the modern Nitro shape — see (4) below. |
| routes    | ✅ done         | Confirm-mode. Lifts the 14 route stubs (admin shell + sign-in) plus the `_byline/route.tsx` pathless layout from `apps/webapp/src/routes/_byline/` into `src/templates/routes/_byline/`, copies into `src/routes/_byline/` at install time. `_byline/route.tsx` imports `@byline/ui/reset.css` + `@byline/ui/styles.css` — scopes the byline UI styles to admin + sign-in. `--admin-path` (default `/admin`) renames the leading directory segment AND rewrites the `/_byline/admin` prefix inside every route-id string literal. The sign-in sibling and the `_byline` layout itself stay fixed. Non-destructive — skips existing files. End-of-phase note explains the pathless-layout pattern + suggests `_front-end/` for isolating the user's existing front-end routes. |
