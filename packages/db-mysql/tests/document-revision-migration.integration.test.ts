...
const native = readFileSync(new URL('../sql/0005_document-revisions.sql', import.meta.url), 'utf8')
...
async function reset(): Promise<void> {
  ...
  await execute(`CREATE TABLE byline_revision_fixture_schedules (
    ...
    CONSTRAINT fixture_check_publish_schedules_suspended_reason
...
async function assertUpgrade() {
  ...
  const columns = await rows(
    "SELECT column_default AS column_default, is_nullable AS is_nullable FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'byline_revision_fixture_documents' AND column_name = 'revision'"
  )
...
describe('document revision native upgrade (mysql)', () => {
  beforeAll(async () => {
    config({ path: '.env.test' })
    const url = process.env.BYLINE_DB_MYSQL_CONNECTION_STRING as string
  ...
  it('resumes after every DDL execution and committed DML boundary', async () => {
    const points = statements.flatMap((statement, index) =>
      (statement.startsWith('EXECUTE ') && statements[index - 2]?.includes('ALTER TABLE')) ||
      statement === 'COMMIT;'
        ? [index + 1]
        : []
    )
    ...
    for (const stop of points) {
      await reset()
      for (const statement of statements.slice(0, stop)) await execute(statement)
      // Lose the session itself, including prepared statements/session variables.
      // MySQL retains completed DDL and rolls back any still-open DML transaction.
      await connection.end()
      const url = process.env.BYLINE_DB_MYSQL_CONNECTION_STRING as string
      assertTestDatabase(url)
      connection = await mysql.createConnection(url)
  ...
  it('refuses an incompatible existing range check instead of accepting its name', async () => {
    await reset()
    await execute('ALTER TABLE byline_revision_fixture_documents ADD revision bigint')
    await execute(
      'ALTER TABLE byline_revision_fixture_documents ADD CONSTRAINT fixture_check_documents_revision CHECK (revision >= 0)'
    )
    await expect(apply()).rejects.toThrow(/stage 2/)
  })
  it('rejects old or defaulted schemas during startup validation', async () => {
    ...
    await execute('ALTER TABLE byline_documents DROP CHECK check_documents_revision')
    let renamed = false
    try {
      await execute(
        'ALTER TABLE byline_documents RENAME COLUMN revision TO revision_legacy_fixture'
      )
      renamed = true
  ...
  it('builds a fresh complete schema from the squashed development baseline', async () => {
    ...
    try {
      ...
      const columns = await rows(
        "SELECT column_default AS column_default, is_nullable AS is_nullable FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'rf_documents' AND column_name = 'revision'"
      )
      expect(columns).toEqual([{ column_default: null, is_nullable: 'NO' }])
      const scheduleColumns = await rows(
        "SELECT column_default AS column_default, is_nullable AS is_nullable FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'rf_document_publish_schedules' AND column_name = 'authorized_revision'"
      )
      expect(scheduleColumns).toEqual([{ column_default: null, is_nullable: 'YES' }])
    } finally {
      await execute('SET FOREIGN_KEY_CHECKS = 0')
      for (const view of views) await execute(`DROP VIEW IF EXISTS \`${view}\``)
      for (const table of tables.toReversed()) await execute(`DROP TABLE IF EXISTS \`${table}\` `)
