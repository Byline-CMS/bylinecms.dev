...
export class DocumentRevisions implements DocumentRevisionStore {
  ...
  async readStructure(params: {
    ...
    const rows = (result as unknown as [Row[], unknown])[0]
    return rows.map((row) => ({
      documentId: row.id,
  ...
  async advance(locked: LockedDocumentRevision): Promise<DocumentRevisionReceipt> {
    ...
    const [changed] = await executor.update(documents).set({ revision: next }).where(predicate)
    const count = changed.affectedRows
    if (count !== 1)
      throw ERR_DATABASE({
        message:
