 ...
 *
 * Validates that the locale='all' round-trip
...
/**
 * Ported from `packages/db-postgres/src/modules/storage/tests/storage-restore.test.ts`.
 */
export function restoreSuite(hooks: ConformanceHooks): void {
  ...
  describe('Document version restore — storage round-trip', () => {
    beforeAll(async () => {
      ...
      adapter = await hooks.createAdapter([RestoreCollectionConfig])
    ...
    it('round-trips multi-locale fields and preserves block _ids when re-emitted via locale: "all"', async () => {
      ...
      const v1Read = await adapter.queries.documents.getDocumentByVersion({
      ...
      const v3Read = await adapter.queries.documents.getDocumentByVersion({
