 ...
 * `byline_document_relationships`. See docs/04-collections/04-document-trees.md.
 *
 ...
 *
 * One test from the original Postgres file does NOT appear here: "serializes
 * direct soft deletion before endpoint validation" exercises raw Postgres row
 * locking (`FOR UPDATE` / `FOR UPDATE NOWAIT` via the pool directly) to force
 * a concurrency race — a white-box test of Postgres's own lock ordering, not
 * an `IDbAdapter`-observable behaviour. It stays in
 * `packages/db-postgres/src/modules/storage/tests/storage-document-tree.test.ts`.
 */
...
/**
 * Ported from `packages/db-postgres/src/modules/storage/tests/storage-document-tree.test.ts`.
 */
export function documentTreeSuite(hooks: ConformanceHooks): void {
  ...
  describe('document-tree commands', () => {
    beforeAll(async () => {
      ...
      adapter = await hooks.createAdapter([TreeCollectionConfig, OtherCollectionConfig])

      const tree = await adapter.commands.collections.create(
        TreeCollectionConfig.path,
        TreeCollectionConfig
      )
      const other = await adapter.commands.collections.create(
    ...
    afterAll(async () => {
      try {
        await adapter.commands.collections.delete(treeCollection.id)
        await adapter.commands.collections.delete(otherCollection.id)
      } catch (error) {
    ...
    it('places roots and children, ordered per-parent', async () => {
      ...
      // Two roots, A then B.
      await adapter.commands.documents.placeTreeNode({
      ...
      const roots = await adapter.queries.documents.getTreeChildren({
        collectionId: treeCollection.id,
        parentDocumentId: null,
      })
      ...
      const aChildren = await adapter.queries.documents.getTreeChildren({
        collectionId: treeCollection.id,
        parentDocumentId: a,
      })
    ...
    it('reorders siblings without changing parent or count', async () => {
      ...
      let kids = await adapter.queries.documents.getTreeChildren({
        collectionId: treeCollection.id,
        parentDocumentId: a,
      })
      ...
      kids = await adapter.queries.documents.getTreeChildren({
        collectionId: treeCollection.id,
        parentDocumentId: a,
      })
    ...
    it('allows only one concurrent placement into the same asserted sibling gap', async () => {
      ...
      expect(
        childIds(
          await adapter.queries.documents.getTreeChildren({
            collectionId: treeCollection.id,
            parentDocumentId: parent,
          })
        )
      ).toEqual([left, winner, right])
      expect(await adapter.queries.documents.getTreeParent({ document_id: loser })).toEqual({
        placed: false,
        parentDocumentId: null,
      })
    })
    ...
    it('rejects placement when the moving node was deleted after the placement snapshot', async () => {
      ...
      await adapter.commands.documents.softDeleteDocument({ document_id: node })
    ...
    it('rejects placement when the target parent was deleted after the placement snapshot', async () => {
      ...
      await adapter.commands.documents.softDeleteDocument({ document_id: parent })
    ...
    it('rejects a deleted target neighbour as a stale conflict', async () => {
      ...
      await adapter.commands.documents.softDeleteDocument({ document_id: neighbour })
    ...
    it('re-parents atomically and updates ancestors', async () => {
      ...
      expect(
        (await adapter.queries.documents.getTreeAncestors({ document_id: c })).map(
          (r) => r.document_id
        )
      ).toEqual([a])
      ...
      const aKids = await adapter.queries.documents.getTreeChildren({
        collectionId: treeCollection.id,
        parentDocumentId: a,
      })
      const bKids = await adapter.queries.documents.getTreeChildren({
        collectionId: treeCollection.id,
        parentDocumentId: b,
      })
    ...
    it('walks ancestors root-first with increasing depth', async () => {
      ...
      const ancestors = await adapter.queries.documents.getTreeAncestors({ document_id: c })
      ...
      expect(await adapter.queries.documents.getTreeAncestors({ document_id: a })).toEqual([])
    })
    ...
    it('getTreeParent distinguishes unplaced, root, and child', async () => {
      ...
      expect(await adapter.queries.documents.getTreeParent({ document_id: stray })).toEqual({
        placed: false,
        parentDocumentId: null,
      })
      ...
      expect(await adapter.queries.documents.getTreeParent({ document_id: root })).toEqual({
        placed: true,
        parentDocumentId: null,
      })
      // Child: placed with its parent id.
      expect(await adapter.queries.documents.getTreeParent({ document_id: child })).toEqual({
        placed: true,
        parentDocumentId: root,
      })

      // Both root and unplaced report empty ancestors — the conflation getTreeParent fixes.
      expect(await adapter.queries.documents.getTreeAncestors({ document_id: root })).toEqual([])
      expect(await adapter.queries.documents.getTreeAncestors({ document_id: stray })).toEqual([])
    })
    ...
    it('getTreeAncestors stops at the first unpublished ancestor in published mode', async () => {
      ...
      expect(
        (
          await adapter.queries.documents.getTreeAncestors({ document_id: leaf, readMode: 'any' })
        ).map((r) => r.document_id)
      ).toEqual([gp, mid])

      // published-mode: the walk stops at the draft MID — it does NOT skip to GP.
      expect(
        await adapter.queries.documents.getTreeAncestors({
          document_id: leaf,
          readMode: 'published',
        })
      ).toEqual([])
      ...
      const draftRoot = await createDoc(
        treeCollection.id,
        TreeCollectionConfig,
        'AE DRoot',
        'draft'
      )
      const pubMid = await createDoc(
        treeCollection.id,
        TreeCollectionConfig,
        'AE PMid',
        'published'
    ...
    it('removeFromTree returns a node to the unplaced state', async () => {
      ...
      await adapter.commands.documents.removeFromTree({
        collectionId: treeCollection.id,
        documentId: c,
      })
      ...
      expect(await adapter.queries.documents.getTreeAncestors({ document_id: c })).toEqual([])

      // Idempotent — removing an already-unplaced node is a no-op.
      await expect(
        adapter.commands.documents.removeFromTree({
          collectionId: treeCollection.id,
          documentId: c,
        })
    ...
    it('getTreeSubtree returns a pre-order depth-first walk with 0-based depth', async () => {
      ...
      const subtree = await adapter.queries.documents.getTreeSubtree({
        collectionId: treeCollection.id,
        rootDocumentId: sa,
      })
      ...
      const shallow = await adapter.queries.documents.getTreeSubtree({
        collectionId: treeCollection.id,
        rootDocumentId: sa,
        maxDepth: 1,
      })
    ...
    it('status-at-edge hides an unpublished node and its whole subtree', async () => {
      ...
      const any = await adapter.queries.documents.getTreeSubtree({
        collectionId: treeCollection.id,
        rootDocumentId: pa,
        readMode: 'any',
      })
      ...
      const published = await adapter.queries.documents.getTreeSubtree({
        collectionId: treeCollection.id,
        rootDocumentId: pa,
        readMode: 'published',
      })
    ...
    it('applies compiled row filters at every tree edge and redacts hidden parents', async () => {
      ...
      const hidden = await createDoc(
        treeCollection.id,
        TreeCollectionConfig,
        'Scoped Hidden Parent'
      )
      ...
      expect(
        await adapter.queries.documents.getTreeAncestors({ document_id: leaf, filters })
      ).toEqual([])
      expect(
        await adapter.queries.documents.getTreeParent({ document_id: hidden, filters })
      ).toEqual({
        placed: false,
        parentDocumentId: null,
      })
      expect(await adapter.queries.documents.getTreeParent({ document_id: leaf, filters })).toEqual(
        {
          placed: true,
          parentDocumentId: null,
          parentRedacted: true,
