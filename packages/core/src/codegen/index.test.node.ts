...
import {
  ...
  type CollectionFieldDataAllLocales,
  defineCollection,
  defineSingleton,
  type InferCollectionRegistry,
  type InferCollectionRegistryAllLocales,
  type InferSingletonRegistry,
  type InferSingletonRegistryAllLocales,
...
describe('emitCollectionTypes', () => {
  it('matches the checked-in all-fields fixture and canonical inferred contracts exactly', () => {
    ...
    expectTypeOf<SingletonPath>().toEqualTypeOf<never>()
  ...
  it('emits a versioned header, verifiable body hash, and canonical formatting', () => {
    ...
    expect(hash).toBe(
      createHash('sha256').update(`@byline/core/codegen:collection-types:v3\n${body}`).digest('hex')
    )
  ...
  it('partitions collection and singleton registries and registers both with the client', () => {
    const definitions = [
      defineCollection({
        path: 'articles',
      ...
      defineSingleton({
        path: 'site-settings',
        label: 'Site settings',
        fields: [{ name: 'siteName', type: 'text', localized: true }],
      }),
    ] as const
    const { source } = emitCollectionTypes(definitions)
    ...
    expect(source).toMatch(/export type SingletonFieldsAllLocalesByPath = \{[^}]*'site-settings'/)
    expect(source).not.toMatch(/export type CollectionFieldsByPath = \{[^}]*'site-settings'/)
    expect(source).not.toMatch(
      /export type CollectionFieldsAllLocalesByPath = \{[^}]*'site-settings'/
    )
    expect(source).toMatch(/export type CollectionFieldsByPath = \{[^}]*articles/)
    expect(source).not.toMatch(/export type SingletonFieldsByPath = \{[^}]*articles/)
    expect(source).toContain("singletons: import('@byline/generated-types').SingletonFieldsByPath")
    expectTypeOf<InferCollectionRegistry<typeof definitions>>().toEqualTypeOf<{
      articles: { title: string }
    }>()
    expectTypeOf<InferCollectionRegistryAllLocales<typeof definitions>>().toEqualTypeOf<{
      articles: { title: string }
    }>()
    expectTypeOf<InferSingletonRegistry<typeof definitions>>().toEqualTypeOf<{
      'site-settings': { siteName: string }
    }>()
    expectTypeOf<InferSingletonRegistryAllLocales<typeof definitions>>().toEqualTypeOf<{
      'site-settings': { siteName: Record<string, string> }
    }>()
  })
