...
describe('public collection lifecycle hooks', () => {
  ...
  it.each([
    ...
    '%s invalidates path-dependent public surfaces and reindexes system-field changes',
  ...
  it.each([
    ...
    '%s uses a collection sweep and reindex for path reconciliation',
    async (collectionPath, hooks) => {
      await invokeHook(hooks.afterSystemFieldsChange, {
        documentId: 'doc-1',
        collectionPath,
        requested: { path: true, availableLocales: false },
  ...
  it.each([
    ...
    '%s preserves structural invalidation for create, status, and unpublish',
    async (collectionPath, hooks, options) => {
  ...
  it.each([
    ...
    '%s removes search and structurally invalidates after deletion',
    async (collectionPath, hooks, options) => {
  ...
  it.each([
    ...
    '%s invalidates previous paths and reindexes ordinary updates',
    async (collectionPath, hooks, options) => {
