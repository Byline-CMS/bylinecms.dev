 ...
 * `server.config.ts`. This file exists only to make PostgreSQL-specific core
...
async function buildBylineCore(): Promise<BylineCore<AdminStore>> {
  ...
  const db = pgAdapter({
    // PostgreSQL DSN used by the inactive comparison runtime.
  ...
  const core = await initBylineCore<AdminStore>({
    ...
    hooks: serverHooks,
    // PostgreSQL document adapter selected by this comparison.
    db,
    // PostgreSQL-backed admin repositories.
    adminStore,
