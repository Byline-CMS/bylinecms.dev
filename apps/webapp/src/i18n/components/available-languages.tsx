...
import { type ContentLocale, contentLocaleLabels, isContentLocale } from '../i18n-config'
...
export function AvailableLanguages({
  ...
  const navigableLocales = advertisedLocales.filter(isContentLocale)

  const handleOnClick = (requestedLocale: ContentLocale) => () => {
    ...
    switchContentLocale(requestedLocale)
  }

  if (navigableLocales.length <= 1) return null
