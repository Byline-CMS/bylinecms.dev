...
Update preview indicator/tooltips, checkbox/source-row explanations, and immediate-save confirmation. Explain complete published-but-unchecked preview, saved-content scope, draft-only completeness, source availability, and immediate checkbox effects on an already published translation. Cover combined content/system-field saves as they actually run: one request and one guarded transaction, system fields first, so a failed save applies neither change; post-commit hook failures (`ERR_DOCUMENT_HOOK_COMMITTED`) are reported after both writes commit. Do not change that save order. (Corrected 2026-09-25: an earlier draft of this plan assumed two requests; see the specification's superseded-premise note.)
...
| U3 / 4–5 | Checkbox writes preserve version count/status and audit semantics; combined save writes system fields first; already-published translation becomes eligible once the save commits, while draft-only translation cannot; a failed combined save applies neither (one guarded transaction, see the specification's implementation note). | Core lifecycle/client admin-save integration and browser. |
...
Update [Content history and document controls](../docs/03-architecture/06-content-history-and-document-controls.md) as part of this phase. Replace its dated descriptions of #102 as planned behavior with verified shipped behavior, including the language policy and combined-save consequence. Preserve the separate status of unresolved re-anchoring, historical replay, and release coordination work. Verify any external specification links resolve to pushed commits before publishing documentation; do not publish links to local-only commits.
