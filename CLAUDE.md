...
- **Phase 5 — status-aware reads (planned)**: `status?: 'published' | 'any'` on `FindOptions` / `PopulateOptions`, plumbed through to `getDocumentsByDocumentIds` as a new filter. `@byline/client` defaults to `'published'`; admin server fns pass `'any'`. Next read-side gap now that Phase 4 has shipped.
