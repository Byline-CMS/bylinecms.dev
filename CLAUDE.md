...
- **Document collections** — `assertActorCanPerform` (`packages/core/src/auth/assert-actor-can-perform.ts`). Called by every write path (`document-lifecycle.*`, `field-upload`) before any storage work, by `@byline/client` `CollectionHandle` for read paths, and by every admin webapp *document-collection* server fn under `apps/webapp/src/modules/admin/collections/*` (`list`, `get`, `history`, `stats` for reads; `create`, `update`, `delete`, `status`, `upload` for writes). Policy: no-context → `ERR_UNAUTHENTICATED`; `actor: null` → permitted only on `read` with `readMode === 'published'`; otherwise `actor.assertAbility('collections.<path>.<verb>')`.
