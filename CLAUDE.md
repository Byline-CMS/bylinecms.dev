...
Byline CMS — an open-source, AI-first headless CMS. Currently at a stable v5.x release (lockstep across the publishable `@byline/*` packages). Licensed MPL-2.0.
...
pnpm test:integration # Integration suite — requires byline_test (see docs/13-testing.md)
pnpm clean            # Clean all workspaces + root node_modules
```

`pnpm test:integration` requires a one-time `pnpm db:init:test` to create the `byline_test` database. The runner auto-migrates it on startup and truncates between test files. CI runs the same suite via `.github/workflows/ci.yml` against a Postgres service container. See [docs/13-testing.md](docs/13-testing.md) for the full story (safety guards, isolation strategy, single-test invocation).
...
cd packages/db-postgres && pnpm test       # Vitest (node mode); test:integration needs Postgres
```
...
- **Flatten** (write): `packages/core/src/storage/storage-flatten.ts` → `flattenFieldSetData()`
- **Reconstruct** (read): `packages/core/src/storage/storage-restore.ts` → `restoreFieldSetData()` — schema-aware, handles meta rows (`_id`, `_type`), locale resolution, and type-correct value extraction inline
- **Store manifest**: the dialect-independent column manifest (`storeColumnManifest`, `storeTableNames`) lives in `packages/core/src/storage/store-manifest.ts`. Each adapter keeps its own SQL generation local — `packages/db-postgres/src/modules/storage/storage-store-manifest.ts` (`storeSelectList()`, `pgNullCast()`) builds the per-store SELECT lists for the UNION ALL from the manifest. Adding a column is a one-line manifest change; positional mismatches are structurally impossible. The adapter-agnostic `fieldTypeToStore` / `fieldTypeToStoreType` mappings live alongside the manifest in `@byline/core` (see "Field → Store Mapping" below) and are re-exported from the adapter module for adapter-internal use.
...
with built-in PostgreSQL and MySQL full-text drivers. The section overview is
[docs/06-search/index.md](docs/06-search/index.md);
the planned attachment-extraction boundary is
`docs/06-search/07-attachment-extraction.md`.
