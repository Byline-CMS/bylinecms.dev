...
6. Reinspect the staged diff and run an appropriate focused verification if the work has not already been verified.
7. Commit with a one-line conventional message. Do not skip hooks or signing.
8. Run `git status --short` and report the commit SHA, subject, verification, and any remaining changes.
